...
export async function POST(
  ...
  try {
    ...
    // Verify user is the SENDER (not the carrier!)
    if (parcel.sender_id !== user.id) {
      return NextResponse.json(
        { error: "Only the sender can pay for this booking" },
        { status: 403 }
      );
    }
    ...
    if (parcel.escrow_status !== "awaiting_payment") {
      return NextResponse.json(
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        payment_type: "sender_booking", // Identify this as sender payment
      },
      payment_intent_data: {
        metadata: {
          parcel_id: parcelId,
    ...
    console.error("Sender checkout error:", error);
    return NextResponse.json(
