...
import { notifyBookingAccepted } from "@/lib/notifications/service";
...
export async function POST(
  ...
  // Fetch parcel with sender and carrier details
  const { data: parcel, error: parcelError } = await supabase
    ...
    .select(
      ...
      *,
      ...
      carrier:profiles!parcels_carrier_id_fkey(*)
    `
    )
    .eq("id", parcelId)
    .single();
  ...
  if (senderAuth?.user?.email && parcel.sender_pin) {
